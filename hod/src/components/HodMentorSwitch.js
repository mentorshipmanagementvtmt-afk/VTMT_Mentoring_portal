...
import { Card, Typography, Select, Button,  Alert } from 'antd';
...
function HodMentorSwitch({ studentId, currentMentorId, onMentorSwitched }) {
  ...
  const handleSwitch = async () => {
    if (selectedMentor === currentMentorId) {
      toast.warning('This student is already assigned to this mentor.');
      return;
    }
    setError('');
    setSubmitting(true);
    ...
    try {
      ...
      toast.success('Mentor successfully reassigned!');
      if (onMentorSwitched) {
        onMentorSwitched();
      }
    } catch (err) {
