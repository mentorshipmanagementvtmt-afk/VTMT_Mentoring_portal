...
import api from 'api';
import { Row, Col, Card, Typography, Spin, Alert, Button, Tag, Space, Avatar, Divider, Popconfirm, Table } from 'antd';
  ...
  const fetchData = async () => {
    ...
    try {
      const [hodRes, mentorRes] = await Promise.all([
        api.get(\`/users/hod/department/\${deptName}\`).catch(() => ({ data: null })),
        api.get(\`/users/mentors?department=\${deptName}\`).catch(() => ({ data: [] }))
      ]);
      setHod(hodRes.data);
...
const adminPath = path.join(__dirname, '../admin/src/pages/DepartmentDetailsPage.js');
const hodPath = path.join(__dirname, '../hod/src/pages/DepartmentDetailsPage.js');

fs.writeFileSync(adminPath, code);
fs.writeFileSync(hodPath, code);
console.log('Created DepartmentDetailsPage.js');